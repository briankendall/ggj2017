    ...
    for(int col = 0; col < entityMap[row].length; col++)
    {
      ...
      for(int thing = 0; thing < trigg.length; thing++)
      {
        if(trigg[thing] && !expos[thing])
        {
          ...
          {
            getRippleRenderer().createRipples((int)((col + 0.5) * toRender[row][col].height), (int)((row + 0.5) * toRender[row][col].width), 100, new color[]{soundForLightColor(thing, 255), soundForLightColor(thing, 255)});
          }
        }
      }
    }
